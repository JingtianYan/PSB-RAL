...
vector<Bot> SIPP_IP::applyPrimitive(int x, int y, int t_lower, int t_upper, Primitive mp) {
    ...
    for(auto mv:mp.mvs){
        ...
        for(auto &it:timeIntervals){
            ...
            // printf("Time interval from : %d -> %d\n", it.first, it.second);
            // for (auto entry: rsrv_tbl[xx][yy]) {
            //     printf("Loop inner intervals from %d -> %d\n", entry.first, entry.second);
                
            // }
            // for (auto entry: debug_rt[instance_ptr->linearizeCoordinate(xx, yy)]) {
            //     printf("RT intervals from %f -> %f\n", entry.t_min, entry.t_max);
            ...
            while (itr_prev->second < t_u) {
                ...
                if (debug_i > 100) {
                    ...
                    for (auto entry: debug_rt[instance_ptr->linearizeCoordinate(xx, yy)]) {
                        printf("RT intervals from %f -> %f, from agent %d\n", entry.t_min, entry.t_max, entry.agent_id);
                        
                    }
                    printf("Potential bug in code\n");
                    printf("In file: %s, with %d agents\n", instance_ptr->agent_fname.c_str(), instance_ptr->num_of_agents);
                    exit(-1);
                    break;
                }
                // printf("Loop inner intervals from %d -> %d\n", itr_prev->first, itr_prev->second);
...
{
public:
    bool operator() (TimeInterval N1, TimeInterval N2) {
        if (N1.t_min > N2.t_min) {
            return true;
        } else {
            return false;
        }
...
void SIPP_IP::RVToRSRV(const ReservationTable& rt)
{
    ...
    for(int i=0; i<MXH; ++i){
        for(int j=0; j<MXW; ++j){
            std::priority_queue<TimeInterval, std::vector<TimeInterval>, IntervalCompare> local_rv;
            // if (instance_ptr->linearizeCoordinate(i, j) == 13866) {
            //     flag = true;
            // }
            for (TimeInterval tmp_interval: rt[instance_ptr->linearizeCoordinate(i, j)]) {
                ...
                // }
                local_rv.push(tmp_interval);
            }
            if (!local_rv.empty()) {
                TimeInterval prev_interval = local_rv.top();
                // if (flag) {
                //     printf("New time interval from %f to %f\n", prev_interval.t_min, prev_interval.t_max);
                // }
                local_rv.pop();
                while (!local_rv.empty()){
                    TimeInterval tmp_interval = local_rv.top();
                    // if (flag) {
                    //     printf("New time interval from %f to %f\n", tmp_interval.t_min, tmp_interval.t_max);
                    // }
                    local_rv.pop();
                    if (tmp_interval.t_min > prev_interval.t_max) {
                        int t_lower = (int) floor(prev_interval.t_min*F);
                        int t_upper = (int) ceil(prev_interval.t_max*F);
                        std::pair<int, int> tmp_t(t_lower, t_upper);
                        rsrv_tbl[i][j].insert({t_lower, t_upper});
                        prev_interval = tmp_interval;
                    } else {
                        prev_interval.t_max = max(prev_interval.t_max, tmp_interval.t_max);
                    }
...
Path SIPP_IP::run(int agent, const ReservationTable& rt, double cutoff_time){
    ...
    // int cntNodes = 0;
    auto search_start_time = Time::now();
    double search_run_time = 0;

    while(!OPEN.empty() && search_run_time<cutoff_time){
        time_s loop_runtime = Time::now() - search_start_time;
		search_run_time = loop_runtime.count();
        // if (search_run_time > counter) {
        //     printf("single loop runtime is: %f\n", search_run_time);
        //     // counter += 1;
        // }
        auto tmp = *OPEN.begin();
        ...
        // printf("If reach the goal\n");

        vector<Bot> succs;
        low_node_expanded++;
        generateSuccessors(x, y, o, v, t_lower, t_upper, succs);
        // printf("Get the successors\n");
...
void SIPP_IP::fillActions() {
    ...
    for (int o = 0; o < MXO; ++o) {
        ...
        tmp.mvs = {Primitive::move(0, 0, 0, 0, 1)};
        tmp.o = (o + 1) % MXO;
        tmp.v = 0;
...
Path SIPP_IP::RetrivePath(int goal_id){
    ...
    for (unsigned int i = 0; i < v.size()-1; i++) {
        ...
        // std::cout << "Primitive is: " << pm << std::endl;
        if (i == 0) {
    ...
    // printf("arrival time for agent: %d is: %f\n", curr_agent.id, result_path.back().leaving_time_tail);
    result_path.back().leaving_time_tail = INF/F;
