...
bool PBS::SolveSingleAgentSIPP_IP(PTNode& node, int agent_id)
{
	...
	// node.printPriorityMap();

	// printValidRT(rt);


	// Path path = sipp_ptr->run(agent_id, rt, node.bezier_solution[agent_id]);
	Path path = sipp_ip_ptr->run(agent_id, rt, cutoff_runtime);
	...
	if(!checkValid(rtdebug, path, agent_id)) {
		...
		printValidRT(rt);
		sipp_ip_ptr->showTable();
		std::cout << "agent " << agent_id <<'\n';
		printPath(path);
		std::cerr << "Invalid path from: " << instance_ptr->agent_fname << ", with agent number: " << instance_ptr->num_of_agents << std::endl;
		exit(-1);
		return false;
	}
	return true;
...
bool PBS::SolveSingleAgentBezier(PTNode& node, int agent_id)
{
	...
	// printRT(rt);
	Path path;
	bool sipp_success = sipp_ptr->run(agent_id, rt, node.bezier_solution[agent_id], path);
...
bool PBS::UpdatePlan(PTNode& node, int index)
{
	...
	list.reverse();
	// printf("Update plan for agent %d\n", index);
	// Showlist(list);
	// node.printPriorityMap();
	...
	for(auto it = list.begin(); it != list.end(); ++it){
		...
		// printf("PBS::UpdatePlan loop for agent %d, is replaned: %d\n", *it, replanned);
		if(replanned){
		...
		if(index == *it){
			// printf("Solve for current collision!\n");
			replanned = true;
			auto start_time = Time::now();
			bool solve_success;
			if (single_agent_solver == "sipp-ip") {
...
bool PBS::GetNextRoundInit()
{
	for (unsigned int i = 0; i < instance_ptr->agents.size(); i++) {
		// printf("[INFO] Retrive solution for agent %d!\n", i);
		...
		if (solution_t < REPLAN_SIZE) {
			...
			// printf("solution of agent %d is: The optimal T is: %f, with control points:\n", i, 
			// 	solution_node->bezier_solution[i].optimal_T);
			...
			double arr_next_dist = CELL_DIS/2 - instance_ptr->agents[i].length/2;
			double leave_prev_dist = CELL_DIS/2 + instance_ptr->agents[i].length/2;
			...
			} else if (tmp_travel_dist < CELL_DIS/2 + instance_ptr->agents[i].length/2) {
				while (tmp_travel_dist < next_node_dist) {
					time_vist_next_node += TIME_STEP_SIZE;
					tmp_travel_dist = tmp_bern.GetVal(time_vist_next_node-instance_ptr->agents[i].earliest_start_time, curr_bezier.control_points);
...
void PBS::updateCost()
{
	solution_cost = 0;
	for (unsigned int i = 0; i < instance_ptr->agents.size(); i++) {
		// printf("Cost for agent %d is: %f\n", i, instance_ptr->agents[i].total_cost);
...
void PBS::saveResults(int all_agents_solved, const string &fileName, const string &instanceName) const
{
	...
	if (!exist) {
		...
		addHeads << "runtime, is solved, #high-level expanded,#high-level generated,#low-level expanded,#low-level generated," <<
			...
			"preprocessing runtime,solver name,instance name,agent_num" << endl;
		addHeads.close();
	}
	...
	stats << runtime << "," << all_agents_solved << ", " <<
		...
		runtime_preprocessing << "," << getSolverName() << "," << instanceName << "," << instance_ptr->num_of_agents << endl;
	stats.close();
}
