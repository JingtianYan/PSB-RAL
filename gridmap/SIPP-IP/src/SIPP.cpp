    ...
    Path& result_path)
{
    ...
    //     "init speed: %f\n",
    //     a_min, a_max, v_min, v_max, curr_agent.init_velocity);
    ...
    if (result_nodes.size() == 2){
        cplex.extract(model);
        std::string model_name = "./output/tpg_model_agent_" + std::to_string(T_optimal) + ".lp";
...
bool SIPP::RecurrentSolver(BezierNode& solution, Path& result_path)
{
    ...
    if (low_valid_solution) {
        ...
        if (result_nodes.size() == 2) {
            SolveBezier(0.51, solution.control_points, result_path);
    ...
    bool success = SolveBezier(solution.optimal_T, solution.control_points, result_path);
    if (success){
