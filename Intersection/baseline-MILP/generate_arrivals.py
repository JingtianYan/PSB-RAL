...
for (laneDirectionIdx, laneDirection) in enumerate(laneDirections):
    arrivalTime = np.random.poisson(7.2,500)
    arrivalTime = np.cumsum(arrivalTime)

    # print(arrivalTime)

    trajectory = np.random.randint(0,10,500)

    vehs = {}
    for i in range(len(arrivalTime)):
        veh = {}
        veh["arrivalTime"] = arrivalTime[i] + np.random.random()
        # 80% vehicle go straigth, 20% turn right or left
        if (trajectory[i] > 1):
            veh["trajectory"] = laneDirection[0]
        else:
            veh["trajectory"] = laneDirection[1]
        veh["id"] = veh["trajectory"][0] + "_" + str(i)
    ...
    vehicleArrival[lanes[laneDirectionIdx]] = vehs
# print(vehicleArrival)
with open('vehicleArrival.json', 'w') as outfile:
